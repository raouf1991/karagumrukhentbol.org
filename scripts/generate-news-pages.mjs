...
const slugify=(v='')=>String(v).toLocaleLowerCase('tr-TR').replace(/ğ/g,'g').replace(/ü/g,'u').replace(/ş/g,'s').replace(/ı/g,'i').replace(/ö/g,'o').replace(/ç/g,'c').normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,90)||'haber';
...
function page(n,all){
 ...
 const published=new Date(n.published_at||n.created_at||Date.now());
 const iso=published.toISOString();
 const date=published.toLocaleDateString('tr-TR',{year:'numeric',month:'long',day:'numeric'});
...
await fs.writeFile('sitemap.xml',sitemap(news),'utf8');
console.log(`Generated ${news.length} static news pages and sitemap.xml.`);
