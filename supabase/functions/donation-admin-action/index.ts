...
async function fetchBytes(url:string){
  ...
  if(!response.ok) throw new Error(`Asset could not be loaded (${response.status}): ${url}`);
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const [regularBytes,boldBytes,scriptBytes,logoBytes]=await Promise.all([
      fetchBytes('https://raw.githubusercontent.com/google/fonts/main/ofl/basic/Basic-Regular.ttf'),
      fetchBytes('https://raw.githubusercontent.com/google/fonts/main/ofl/russoone/RussoOne-Regular.ttf'),
      fetchBytes('https://raw.githubusercontent.com/google/fonts/main/ofl/greatvibes/GreatVibes-Regular.ttf'),
    ...
    centerText('TEŞEKKÜR BELGESİ',405,34,bold,black);
    ...
    const mail=await fetch('https://api.resend.com/emails',{
      ...
      body:JSON.stringify({from,to:[a.email],subject:'Karagümrük Hentbol - Teşekkür Belgesi',html,attachments:[{filename:`Karagumruk-Tesekkur-Belgesi-${cert}.pdf`,content:attachment}]})
    });
