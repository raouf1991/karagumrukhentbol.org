(()=>{
  ...
  const MEMBERSHIP_TYPES=['Yetişkin Üye','Tam Üyelik','Yönetim Kurulu Üyesi','Onursal Üyelik','Ziyaretçi Üyeliği','Sporcu Üyeliği'];
  ...
  function printMembership(row){
    const w=window.open('','_blank','width=1050,height=760');if(!w)return notify('Yazdırma penceresi açılamadı.',true);
    const qr='https://api.qrserver.com/v1/create-qr-code/?size=170x170&data='+encodeURIComponent('https://karagumrukhentbol.org/?membership='+encodeURIComponent(row.membership_number||''));
    w.document.write(`<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>Üyelik Kartı</title><style>*{box-sizing:border-box}body{margin:0;background:#eceff2;font-family:Arial,sans-serif;color:#fff;padding:28px}.sheet{display:grid;grid-template-columns:1fr 1fr;gap:24px;max-width:1000px;margin:auto}.card{aspect-ratio:1.586/1;border-radius:24px;overflow:hidden;position:relative;background:linear-gradient(135deg,#08090a,#171717 64%,#260000);box-shadow:0 18px 55px #0004}.card:before{content:"";position:absolute;inset:0;background:linear-gradient(125deg,transparent 0 66%,#d50909 66% 69%,transparent 69% 73%,#850000 73% 77%,transparent 77%);opacity:.9}.front{padding:30px}.head{display:flex;align-items:center;gap:15px;position:relative;z-index:2}.logo{width:84px;height:84px;object-fit:contain;border-radius:50%;background:#fff}.club{font-weight:900;font-size:23px;line-height:1.05}.body{display:grid;grid-template-columns:118px 1fr 100px;gap:18px;align-items:center;margin-top:28px;position:relative;z-index:2}.photo{width:118px;height:142px;object-fit:cover;border-radius:10px;border:2px solid #fff}.placeholder{width:118px;height:142px;border-radius:10px;background:#333;display:grid;place-items:center;color:#aaa}.name{font-size:22px;font-weight:900;margin-bottom:13px}.meta{line-height:1.75;font-size:15px}.meta b{display:inline-block;min-width:92px}.qr{width:92px;height:92px;background:#fff;padding:5px}.slogan{position:absolute;left:30px;bottom:24px;font-weight:900;font-style:italic;font-size:20px;z-index:2}.slogan em{color:#ef1c1c;font-style:italic}.back{display:grid;place-items:center;text-align:center}.back .logo{width:190px;height:190px;position:relative;z-index:2}.back .url{position:absolute;bottom:30px;z-index:2}.back .motto{position:absolute;right:28px;bottom:58px;text-align:left;font-weight:900;font-style:italic;font-size:22px;line-height:.85;z-index:2}.back .motto em{color:#ef1c1c;font-style:italic}@media print{body{background:#fff;padding:8mm}.card{box-shadow:none}.sheet{gap:10mm}}</style></head><body><div class="sheet"><section class="card front"><div class="head"><img class="logo" src="https://karagumrukhentbol.org/assets/club-logo.png"><div class="club">KARAGÜMRÜK<br>HENTBOL SPOR KULÜBÜ</div></div><div class="body">${row.photo_url?`<img class="photo" src="${esc(row.photo_url)}">`:'<div class="placeholder">FOTOĞRAF</div>'}<div><div class="name">${esc(row.full_name||'—')}</div><div class="meta"><b>Üyelik No</b>: ${esc(row.membership_number||'—')}<br><b>Üyelik Türü</b>: ${esc(row.membership_type||'—')}<br><b>Geçerlilik</b>: ${fmt(row.valid_until)}</div></div><img class="qr" src="${qr}"></div><div class="slogan">DAİMA DAHA <em>İLERİ</em></div></section><section class="card back"><img class="logo" src="https://karagumrukhentbol.org/assets/club-logo.png"><div class="motto">DAİMA<br>DAHA<br><em>İLERİ</em></div><div class="url">www.karagumrukhentbol.org</div></section></div><script>onload=()=>setTimeout(()=>print(),700)<\/script></body></html>`);w.document.close();
  }
