...
$('#loginForm')?.addEventListener('submit',async e=>{
  e.preventDefault();const btn=$('#loginButton');loginError('');btn.disabled=true;btn.textContent='Giriş yapılıyor...';
  try{const email=$('#email').value.trim();const password=$('#password').value;const {data,error}=await withTimeout(db.auth.signInWithPassword({email,password}),15000);if(error)throw error;if(!data.user)throw new Error('Kullanıcı bilgisi alınamadı.');showPanel(data.user);status('Giriş başarılı');await loadAll();}
...
$('#newsForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;setSubmitState(f,true,'Yükleniyor...');try{const d=formObject(f);delete d.image_file;const file=f.elements.image_file.files[0];d.image_url=await uploadImage(file,'news');d.published=f.elements.published.checked;d.published_at=new Date().toISOString();const {error}=await db.from('news').insert(d);if(error)throw error;toast('Haber ve görsel eklendi');f.reset();await loadAll();}catch(err){toast(err.message||String(err),true);}finally{setSubmitState(f,false);}});
...
$('#playerForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;setSubmitState(f,true,'Yükleniyor...');try{const d=formObject(f);delete d.image_file;const file=f.elements.image_file.files[0];d.image_url=await uploadImage(file,'players');d.number=emptyToNull(d.number)==null?null:Number(d.number);d.active=f.elements.active.checked;const {error}=await db.from('players').insert(d);if(error)throw error;toast('Oyuncu ve fotoğraf eklendi');f.reset();await loadAll();}catch(err){toast(err.message||String(err),true);}finally{setSubmitState(f,false);}});
...
$('#sponsorForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;setSubmitState(f,true,'Yükleniyor...');try{const d=formObject(f);delete d.image_file;const file=f.elements.image_file.files[0];d.logo_url=await uploadImage(file,'sponsors');d.sort_order=Number(d.sort_order||0);d.active=true;const {error}=await db.from('sponsors').insert(d);if(error)throw error;toast('Sponsor ve logo eklendi');f.reset();await loadAll();}catch(err){toast(err.message||String(err),true);}finally{setSubmitState(f,false);}});
